...
def test_end_to_end_single_data_chunk_ack_and_file_write(tmp_path):
    ...
    # Raw sockets send to (ip, 0) - port is in UDP header
    assert sent_addr[0] == addr[0]  # IP matches

    # Extract custom packet from raw packet (skip IP + UDP headers)
    # IP header is variable length, UDP header is 8 bytes
    # For this test, we need to extract the custom packet portion
    # Skip IP header (20 bytes min) + UDP header (8 bytes) = 28 bytes minimum
    ihl = (sent_bytes[0] & 0x0F) * 4
    custom_packet = sent_bytes[ihl + 8:]
    ack_pkt = decode_packet(custom_packet)
...
def test_end_to_end_out_of_order_then_in_order_reassembles_file(tmp_path):
    ...
    # Extract custom packet from raw packet
    ihl = (sent_bytes[0] & 0x0F) * 4
    custom_packet = sent_bytes[ihl + 8:]
    ack_pkt = decode_packet(custom_packet)
    assert ack_pkt is not None
